...
TEXTE = [
    ...
    ('Vor meiner Arbeit als Coach habe ich Privatkunden bei einer großen Bank beraten. Ich habe gesehen, mit welchen Fragen Menschen dorthin kommen — und wie oft sie mit dem Gefühl wieder gehen, etwas nicht verstanden zu haben, das sie eigentlich hätten verstehen dürfen.',
     'Before my work as a coach I advised private clients at a large bank. I saw the questions people bring through the door — and how often they leave feeling they hadn’t understood something they had every right to understand.',
    ...
    ('Daneben baue ich seit über zehn Jahren mehrere Immobilienunternehmen mit auf. Ich weiß also nicht nur, wie man über Vermögen spricht, sondern wie es sich anfühlt, eines aufzubauen: die Zweifel am Anfang, die langen Strecken ohne sichtbaren Fortschritt, die Entscheidungen, die man trifft, bevor man sicher ist.',
     'Alongside that, I’ve spent more than ten years helping build several real-estate companies. So I know not only how to talk about wealth, but what it feels like to build it: the doubt at the start, the long stretches with nothing visible to show, the decisions you make before you’re sure.',
    ...
    # --- hnc: Abschnitt auf der Startseite ---------------------------------
    ('Zu mir kommt man auf zwei Wegen. Die meisten über das Gespräch. Und manche über den Körper, weil nicht jeder Mensch über Worte erreichbar ist und sich manches leichter löst, wenn man es nicht erklären muss. Beides gehört bei mir zusammen; was zu dir passt, finden wir gemeinsam heraus.',
     'People come to me by two routes. Most through conversation. And some through the body, because not everyone can be reached through words and some things come loose more easily when you don’t have to explain them. With me the two belong together; which one suits you, we work out between us.',
     'Chega-se a mim por dois caminhos. A maioria pela conversa. E alguns pelo corpo, porque nem toda a gente se deixa alcançar pelas palavras e há coisas que se soltam mais facilmente quando não é preciso explicá-las. Comigo os dois andam juntos; qual deles te serve, descobrimos em conjunto.'),
    ('>Der zweite Weg<', '>The second route<', '>O segundo caminho<'),
    ('<h2>Wenn Worte nicht reichen.</h2>',
     '<h2>When words aren’t enough.</h2>',
     '<h2>Quando as palavras não chegam.</h2>'),
    ('Manches lässt sich nicht bereden. Nicht, weil man nicht wollte, sondern weil es keine Worte dafür gibt — oder weil sie an der Sache vorbeigehen.',
     'Some things can’t be talked through. Not because you wouldn’t want to, but because there are no words for them — or because the words miss the point.',
     'Há coisas que não se resolvem a falar. Não por falta de vontade, mas porque não há palavras para elas — ou porque as palavras passam ao lado.'),
    ('Dann arbeite ich mit den Händen. <em>hnc</em> ist eine ruhige, manuelle Arbeit am Nervensystem, in der Arbeitsweise vergleichbar mit Physiotherapie oder Osteopathie. Du liegst, du bleibst angezogen, du musst nichts erklären.',
     'Then I work with my hands. <em>hnc</em> is quiet, hands-on work with the nervous system, close in approach to physiotherapy or osteopathy. You lie down, you stay dressed, you don’t have to explain anything.',
     'Então trabalho com as mãos. O <em>hnc</em> é um trabalho manual e calmo com o sistema nervoso, com uma abordagem próxima da fisioterapia ou da osteopatia. Ficas deitado, vestido, e não tens de explicar nada.'),
    ('Eine Stunde, vor Ort in Düsseldorf. Eine Grundbehandlung sind meist zwei Sitzungen.',
     'One hour, in person in Düsseldorf. A basic course is usually two sessions.',
     'Uma hora, presencial em Düsseldorf. O trabalho de base são normalmente duas sessões.'),
    ('>Mehr zu hnc</a>', '>More on hnc</a>', '>Mais sobre o hnc</a>'),
    ('alt="hnc — ruhige Arbeit am Nervensystem"',
     'alt="hnc — quiet work with the nervous system"',
     'alt="hnc — trabalho calmo com o sistema nervoso"'),

    # --- hnc: eigene Seite -------------------------------------------------
    ('>Körperarbeit<', '>Bodywork<', '>Trabalho corporal<'),
    ('Wenn Worte<br><em>nicht reichen.</em>',
     'When words<br><em>aren’t enough.</em>',
     'Quando as palavras<br><em>não chegam.</em>'),
    ('Manche Menschen erreicht man über das Gespräch. Andere über den Körper. Für den zweiten Weg arbeite ich mit hnc.',
     'Some people are reached through conversation. Others through the body. For the second route I work with hnc.',
     'Há pessoas que se alcançam pela conversa. Outras pelo corpo. Para o segundo caminho trabalho com hnc.'),
    ('Was hnc ist', 'What hnc is', 'O que é o hnc'),
    ('Eine ruhige, manuelle Arbeit am Nervensystem.',
     'Quiet, hands-on work with the nervous system.',
     'Um trabalho manual e calmo com o sistema nervoso.'),
    ('hnc steht für human neuro cybrainetics. Ich arbeite mit den Händen, sehr ruhig, an Punkten, über die das Nervensystem erreichbar ist. In der Arbeitsweise ist das vergleichbar mit Physiotherapie oder Osteopathie — nur dass es nicht um einen bestimmten Muskel oder ein bestimmtes Gelenk geht, sondern um die Spannung dahinter.',
     'hnc stands for human neuro cybrainetics. I work with my hands, very quietly, at points through which the nervous system can be reached. In approach it is close to physiotherapy or osteopathy — except that it isn’t about a particular muscle or a particular joint, but about the tension behind it.',
     'hnc significa human neuro cybrainetics. Trabalho com as mãos, muito devagar, em pontos por onde o sistema nervoso é alcançável. Na abordagem é próximo da fisioterapia ou da osteopatia — só que não se trata de um músculo ou de uma articulação em concreto, mas da tensão que está por trás.'),
    ('Menschen nehmen unterschiedlich auf: die einen über das Hören, die anderen über das Sehen, wieder andere über das Spüren. Wer im Gespräch nicht weiterkommt, kommt manchmal über den Körper an einen Punkt, den Worte nicht erreichen — weil sich dabei nichts erklären lässt und auch nichts erklärt werden muss.',
     'People take things in differently: some through hearing, some through seeing, others through feeling. When talking gets you nowhere, the body sometimes reaches a place that words don’t — because nothing there can be explained, and nothing has to be.',
     'As pessoas recebem de maneiras diferentes: umas pela audição, outras pela visão, outras pelo tato. Quem não avança na conversa chega por vezes, através do corpo, a um ponto onde as palavras não chegam — porque ali não há nada a explicar, nem é preciso.'),
    ('Du bleibst dabei angezogen und ansprechbar. Es wird nichts eingerenkt, nichts gedehnt, nichts geknetet.',
     'You stay dressed and awake throughout. Nothing is clicked into place, nothing is stretched, nothing is kneaded.',
     'Ficas vestido e desperto durante todo o tempo. Nada é destorcido, nada é esticado, nada é amassado.'),
    ('Eine Stunde, im Liegen.', 'One hour, lying down.', 'Uma hora, deitado.'),
    ('>Zu Beginn<', '>At the start<', '>No início<'),
    ('>Danach<', '>Afterwards<', '>No fim<'),
    ('<h3>Kurzes Gespräch</h3>', '<h3>A short conversation</h3>', '<h3>Uma conversa curta</h3>'),
    ('<h3>Die Arbeit selbst</h3>', '<h3>The work itself</h3>', '<h3>O trabalho em si</h3>'),
    ('<h3>Zeit zum Ankommen</h3>', '<h3>Time to land</h3>', '<h3>Tempo para aterrar</h3>'),
    ('Du erzählst mir, was dich herführt und was körperlich gerade da ist. Das dauert ein paar Minuten, keine Anamnese, kein Fragebogen.',
     'You tell me what brings you here and what is going on in your body. That takes a few minutes — no case history, no questionnaire.',
     'Contas-me o que te traz aqui e o que se passa no teu corpo. São uns minutos — sem histórico clínico, sem questionário.'),
    ('Du liegst, ich arbeite mit den Händen. Die meisten werden dabei sehr ruhig. Manche schlafen ein. Reden musst du nicht.',
     'You lie down, I work with my hands. Most people go very quiet. Some fall asleep. You don’t have to talk.',
     'Ficas deitado, eu trabalho com as mãos. A maioria fica muito calma. Há quem adormeça. Não tens de falar.'),
    ('Am Ende braucht es meist einen Moment. Ich sage dir, was mir aufgefallen ist, und wir schauen, ob eine zweite Sitzung sinnvoll ist.',
     'At the end most people need a moment. I tell you what I noticed, and we look at whether a second session makes sense.',
     'No fim, a maioria precisa de um momento. Digo-te o que me chamou a atenção e vemos se faz sentido uma segunda sessão.'),
    ('Eine Grundbehandlung besteht in der Regel aus zwei Sitzungen — die zweite baut auf der ersten auf. Ob es danach weitergeht, entscheidest du.',
     'A basic course is usually two sessions — the second builds on the first. Whether it goes on after that is your decision.',
     'O trabalho de base são normalmente duas sessões — a segunda assenta na primeira. Se continua depois disso, decides tu.'),
    ('>Einzelsitzung · {{preise.hnc_dauer}} · vor Ort in Düsseldorf · {{preise.hnc}}<',
     '>Single session · {{preise.hnc_dauer}} · in person in Düsseldorf · {{preise.hnc}}<',
     '>Sessão individual · {{preise.hnc_dauer}} · presencial em Düsseldorf · {{preise.hnc}}<'),
    ('Was hnc nicht ist.', 'What hnc is not.', 'O que o hnc não é.'),
    ('hnc ist keine Heilbehandlung. Ich stelle keine Diagnosen, behandle keine Krankheiten und ersetze weder Ärztin noch Therapeut. Wenn etwas in ärztliche Hände gehört, sage ich dir das.',
     'hnc is not medical treatment. I make no diagnoses, I treat no illnesses, and I am no substitute for a doctor or a therapist. If something belongs in medical hands, I’ll tell you so.',
     'O hnc não é tratamento médico. Não faço diagnósticos, não trato doenças e não substituo médico nem terapeuta. Se algo pertence a mãos médicas, digo-to.'),
    ('Ich verspreche auch keine Ergebnisse. Was ich anbiete, ist eine Stunde ruhige Arbeit und meine Aufmerksamkeit — was daraus entsteht, ist bei jedem anders.',
     'Nor do I promise results. What I offer is an hour of quiet work and my full attention — what comes of it is different for everyone.',
     'Também não prometo resultados. O que ofereço é uma hora de trabalho calmo e a minha atenção — o que daí resulta é diferente para cada pessoa.'),
    ('Es gibt keine Wunderheilung, aber wer ein paar Themen in seinem „Rucksack des Lebens" mit sich herum schleppt, die kein traditioneller Doktor mit Skalpell und Tabletten lösen kann, ist hier an der richtigen Adresse!',
     'There is no miracle cure, but anyone carrying a few things around in their “rucksack of life” that no traditional doctor can fix with a scalpel and pills has come to the right place!',
     'Não há curas milagrosas, mas quem carrega umas quantas coisas na sua «mochila da vida» que nenhum médico tradicional resolve com bisturi e comprimidos está no sítio certo!'),
    ('Malte W. · Google-Rezension', 'Malte W. · Google review', 'Malte W. · avaliação no Google'),
    ('Am einfachsten probierst du es aus.', 'The simplest thing is to try it.', 'O mais simples é experimentares.'),
    ('Wenn du nicht weißt, ob das etwas für dich ist: schreib mir, dann klären wir das in fünf Minuten.',
     'If you don’t know whether this is for you: write to me and we’ll settle it in five minutes.',
     'Se não sabes se isto é para ti: escreve-me e resolvemos isso em cinco minutos.'),
]
